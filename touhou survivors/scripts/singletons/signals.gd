extends Node
class_name signals

signal damage_popup
signal spawn_pickup
signal despawn_offscreen_enemies
signal update_power
signal update_faith
signal add_weapon
signal remove_weapon
signal modify_weapon
signal show_tooltip
signal hide_tooltip
signal leveling_up
signal modify_player_speed
signal modify_player_scale
signal gap_teleport
signal gap_camera_tween
signal gap_finish
signal gap_close
signal increase_inventory
signal player_not_moving_in_pause
signal increase_threat
signal update_afterimage
signal general_values
signal current_power
signal current_faith
signal next_lvl_update
signal weapon_add_child
signal update_orb_count
signal reduce_orb_count
signal camera_is_still
signal revert_player_speed
signal enemy_death_sfx
signal hit_sfx
signal game_over
signal spawn_inventory_items
signal show_icon_highlight
signal spawn_bullet
signal current_hp
signal increase_max_hp
signal update_crystal
signal current_crystal
signal show_right_click_tip
signal current_crystal_count
signal decrease_crystal_count
signal not_enough_crystals
signal show_blocked_spaces
signal reroll_gap
signal check_matching_sets
signal show_spell_card_right_click
signal show_hand_cursor
signal taking_damage
signal spawn_mon
signal increase_mon
signal total_power
signal mon_sfx
signal faith_sfx
signal crystal_sfx
signal tornado_sfx
signal player_damage_sfx
signal explosion_sfx
signal item_grab_sfx
signal item_place_sfx
signal rotate_sfx
signal not_enough_crystals_sfx
signal remove_blocked_space_sfx
signal gohei_sfx
signal amulet_sfx
signal bat_sfx
signal camera_sfx
signal frog_explosion_sfx
signal haniwa_sfx
signal icicle_explode_sfx
signal keystone_large_sfx
signal keystone_small_sfx
signal leaf_fan_sfx
signal arrow_sfx
signal star_sfx
signal gap_open_sfx
signal fireball_sfx
signal fireball_split_sfx
signal water_sfx
signal megaphone_blast_sfx
signal megaphone_bullet_sfx
signal master_spark_sfx
signal mini_hakkero_sfx
signal mochi_mallet_bash_sfx
signal mochi_mallet_swing_sfx
signal mushroom_explode_sfx
signal raincloud_sfx
signal persuasion_needle_sfx
signal roukanken_flurry_sfx
signal roukanken_spin_sfx
signal sake_swing_sfx
signal sake_bottle_break_sfx
signal shanghai_attack_sfx
signal knife_throw_sfx
signal tripod_swing_sfx
signal yinyang_sfx
signal youkai_umbrella_jump_sfx
signal holding_item
signal game_start_complete
signal go_to_main_menu
signal delete_player
signal player_deleted
signal delete_ui
signal ui_deleted
signal open_options_menu
signal close_options_menu
signal options_menu_closed
signal save_video_settings
signal game_over_music
signal death_explosion_sfx
signal death_sfx
signal stop_weapons
signal show_fps
